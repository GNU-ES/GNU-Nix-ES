...
--user='root' \
--volume "$(pwd)":/code \
alpine-gnu-nix-es:0.0.1 bash -c 'stat --format="%a" /nix/store/.links'
...
--user='pedroregispoar' \
--volume "$(pwd)":/code \
alpine-gnu-nix-es:0.0.1 bash -c ' /nix/store/jbryisjscy1mrkfnk8zqgcqynxbvrhci-nix-2.3.6/bin/nix-env --install --attr nixpkgs.git'