...
let
    ...
    inherit (pkgs) bashInteractive coreutils cacert nix findutils file which su sudo shadow;
...
#    shadow = pkgs.shadow.override { pam = null; };
    ...
    path = buildEnv {
        name = "system-path";
        paths = [ findutils file bashInteractive coreutils nix which shadow su sudo];
    };
